...
const Dashboard = () => {
  ...
  const handleLogout = () => {
    ...
    setTimeout(() => {
      logout();
    }, 1500); // tiempo visual del loading
  };
  ...
  const currentLabel =
    allowedMenu.find(m => location.pathname.includes(m.path))?.label || "Panel";
    ...
    <div className="drok-container d-flex vh-100 overflow-hidden">
      ...
      <aside className={`drok-sidebar ${showSidebar ? 'mobile-on' : ''}`}>
        ...
        <div className="p-3 border-top border-dark border-opacity-50">
          <button className="drok-logout-solid" onClick={handleLogout}>
      ...
      {showSidebar && (
        <div className="drok-blur-overlay d-md-none" onClick={() => setShowSidebar(false)}></div>
      )}
      ...
      {/* LOADING AL CERRAR SESIÓN */}
      {isLoggingOut && <LoadingScreen text="Cerrando sesión..." />}
      ...
      <style>{`
        ...
        .drok-logo { color: #fff; font-weight: 900; font-size: 1.6rem; margin: 0; }
        ...
        .drok-header { height: 65px; background: #fff; border-bottom: 1px solid #eee; }
        .header-title { color: #000; }
        .drok-burger { background: none; border: none; font-size: 2rem; color: #000; }

        @media (max-width: 768px) {
          .drok-header { background: #000 !important; border-bottom: 1px solid #111; }
          .header-title { color: #fff !important; }
          .drok-burger { color: #fff !important; }
